...
| 11 | Menu mobile, l'architecture | le panneau était un plan de site : trois dépliants, un Reste de seize lignes, la langue et le thème au poids des portes, et un demi-écran vide au-dessus · il répond maintenant à une question, où veux-tu aller, en dix lignes lues de haut en bas : cinq portes en liens directs (le plan passe devant les séances, l'ordre raconte le produit ; Mes semaines sort de Mon plan, où la personne sans plan ne la trouvait pas), deux OUTILS que les réglages peuvent masquer (le générateur de parcours reste au pied de page et dans la palette) (`module` sur `NavSection`), la recherche et deux liens de service sous le filet · les pages d'une porte vivent sur sa page d'accueil, en rail (`HubNav.tsx`, sous 1024px, même donnée que le menu déroulant du bureau) · `nav-coverage.test.ts` refuse la onzième ligne |
