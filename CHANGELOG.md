...
### Added
- A session shares as an image in the house style, in the four sizes the week already has, first in the picker. The forty pastiches stay, but the sheet drawn in the app's own language, paper, ink, every edge ruled, the ramp for the zones and the wordmark's one vermillon dot, was one portrait template with a drawing and no structure. The four new sheets print what a session is: the name, its facts in mono (category, difficulty, duration, dominant zone, load), the profile as a frieze on the ramp, every block as a ruled row under its phase with its figure on the right (8 × 3min, 3 × 12, 20min) and its zone, the time in zones as a ruled bar, and the first coaching tip. `Sheet` (1080 × 1350) and `Story` (1080 × 1920) carry the two runners in ink on what a short session leaves; `Square` (1080 × 1080) pins the split above the footer; `Landscape` (1200 × 675) sets the blocks beside the split and the tip, and past six blocks prints them in two columns. A sheet cannot grow, so a dense session takes its room from the rows' type in steps, and the tip and the runners go before any block is cut. Verified on a one-block recovery ride, a four-block endurance run and a thirteen-block fartlek, in the four sizes
