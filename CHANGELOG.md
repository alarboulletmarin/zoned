...
### Added
- The session structure saves as an image of its own. The phase cards, step by step, are the block people screenshot to take to a session or send to someone, and the existing session PNG renders the whole card instead: the title, the profile and the zone table, but not that breakdown. A `Download the structure` action sits under the phases and captures the LIVE block, so the image carries exactly what the page shows, personalised paces included
