...
### Changed
- The complementary activity form asks its one required question in one gesture. The duration was a single field in MINUTES, which demanded a mental conversion before the first keystroke: nobody thinks of their bike commute as 85 minutes, they think 1h25. It splits into an HOURS field and a MINUTES field, and the parsing stays tolerant rather than strict, 90 in the minutes field is a valid duration and normalises to 1h30 on blur instead of being refused. Typing the hours jumps to the minutes as soon as a second digit becomes impossible (two digits entered, or a first digit of 3 or more, since an activity fits inside its date), and backspace on an empty minutes field goes back. Storage does not move: `durationMin` is still minutes, the conversion lives at the form's edge and nowhere else
