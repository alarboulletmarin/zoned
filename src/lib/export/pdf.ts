...
export async function exportToPDF(
  ...
  const content: Content = [
    ...
    {
      columns: [
        { text: `${isEn ? "Category" : "Catégorie"}: ${category}`, style: "metadata" },
        { text: `${isEn ? "Difficulty" : "Difficulté"}: ${difficulty}`, style: "metadata" },
        { text: `${isEn ? "Duration" : "Durée"}: ${duration} min`, style: "metadata" },
      ],
      margin: [0, 0, 0, 20],
    },

    // Warmup
    { text: isEn ? "Warmup" : "Échauffement", style: "sectionHeader" },
    {
      table: {
        headerRows: 1,
        widths: ["*", 50, 40, 35, 50],
    ...
    // Main Set
    { text: isEn ? "Main Set" : "Corps de séance", style: "sectionHeader" },
    {
      table: {
        headerRows: 1,
        widths: ["*", 50, 40, 35, 50],
    ...
    {
      ul: tips.length > 0 ? tips : [isEn ? "No specific tips" : "Pas de conseils spécifiques"],
      margin: [0, 0, 0, 15],
    },
    ...
    {
      ul: mistakes.length > 0 ? mistakes : [isEn ? "No specific warnings" : "Pas d'erreurs spécifiques"],
      margin: [0, 0, 0, 15],
    },
