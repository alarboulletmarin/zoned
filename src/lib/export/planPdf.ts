...
export async function exportPlanToPDF(
      ...
      columns: [
        {
          ...
          stack: [
            {
              text: `${isEn ? "Start" : "Debut"}: ${formatDate(plan.config.createdAt, isEn)}`,
              style: "metadata",
            },
            {
            ...
            {
              text: `${isEn ? "Duration" : "Duree"}: ${plan.totalWeeks} ${isEn ? "weeks" : "semaines"}`,
              style: "metadata",
            },
          ],
        },
        {
          ...
          stack: [
            ...
            ...(plan.raceTimePrediction
              ...
              : []),
            {
              text: `${isEn ? "Sessions/week" : "Seances/sem"}: ${plan.config.daysPerWeek}`,
              style: "metadata",
            },
          ],
        },
  ...
  // Phase overview
  content.push(
    { text: isEn ? "Training Phases" : "Phases d'entrainement", style: "sectionHeader" },
    {
      table: {
        headerRows: 1,
        widths: ["*", "auto", "auto"],
  ...
  for (const week of plan.weeks) {
    ...
    if (week.sessions.length === 0) {
      content.push({
        text: isEn ? "No sessions this week" : "Aucune seance cette semaine",
        italics: true,
        color: "#888",
        margin: [0, 4, 0, 12],
      });
    ...
    const tableHeader: TableCell[] = [
      ...
      { text: isEn ? "Key" : "Cle", style: "tableHeader" },
    ];
